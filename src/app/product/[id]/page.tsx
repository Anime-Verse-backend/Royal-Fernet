...
export default function ProductDetailPage() {
  ...
  React.useEffect(() => {
    // Add a guard to ensure `id` is available
    if (!id) return;
    ...
    const getProduct = async () => {
      ...
      if (!fetchedProduct) {
        // This will render the not-found.tsx page if the product doesn't exist.
        return notFound();
      }
  ...
  if (!product) {
    // This will be hit if notFound() is called, or if there's an issue before loading finishes.
    return notFound();
  }
